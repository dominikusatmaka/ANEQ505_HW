...
```
metadata <- read_tsv("/Users/dominikusraditya/Documents/ANEQ505_HW/cow_r/03_metadata/cow_metadata.txt")
```

**Read in alpha diversity data ~={red}(1 point)=~**
- Fill in the file path you used in the R Markdown to load the shannon data
```
shannon <- read_tsv("/Users/dominikusraditya/Documents/ANEQ505_HW/cow_r/04_code/alpha_div/shannon.tsv")
```
...
```
uw_unifrac <- read_tsv("/Users/dominikusraditya/Documents/ANEQ505_HW/cow_r/04_code/beta_div/unweighted_unifrac.txt")
```

**Load in tabulated results ~={red}(1 point)=~**
- Fill in the file path you used in the R Markdown to load the tabulated_results.tsv
```
tabulated_results <- read_tsv("/Users/dominikusraditya/Documents/ANEQ505_HW/cow_r/04_code/taxonomy/tabulated_results.tsv")
```
...
1. Describe one way to get data from your qiime2 outputs into a format that can be used for R. 
	One way is to use qiime tools export to export QIIME2 artifacts (.qza files) into plain text or biom format and then convert it to a TSV using biom convert. The resulting TSV can then be read directly into R using read_tsv(). QIIME2 visualization files (.qzv) also can be unzipped and the underlying data files (like TSV or CSV files) can be extracted and loaded into R.
...
5. What was the most enriched genus in skin compared to fecal, and what was the most depleted genus in skin compared to fecal (make sure adjusted p is set to less than 0.05)?
	_Atopostipes_ is the most enriched genus in skin relative to fecal, with an LFC of +4.209 and an adjusted q-value of < 0.0001. 
...
3. what are 2 features that are low in nasal?
	Features that appear darkest purple but not black (lowest abundance but not zero) in the nasal column are genus Cellulosilyticum (species Cellulosilyticum ruminicola) and Ruoffia (Actually it is really hard to determined is it a black or dark purple since there are several genus nearly has the same color, so I am not sure to my answer but I am trying my best to identify), these are gut-associated taxa that are largely absent from the nasal cavity, which has a very different microbial environment.
...
4. what is the accuracy of your model, and if the accuracy of the classifier is high, what does that suggest about the microbial compositions of each site?
	The classifier achieved a baseline accuracy of 88.23% (0.8823) (compared to a baseline of 35.29% (0.3529), giving an accuracy ratio of 2.5×), indicating a high classifier. Looking at the confusion matrix from the TSV: fecal, nasal, oral, and skin samples were classified with 100% accuracy, while udder was 33.3% correct with 66.7% misclassified as skin. This pattern suggests that microbial communities differ substantially across body sites, making them relatively easy to distinguish based on their composition. The high overall accuracy, especially the perfect scores for fecal, nasal, oral, and skin, suggests that those body sites have highly distinct and consistent microbial compositions that the model can reliably distinguish. The confusion between skin and udder makes biological sense, since those two surface sites share similar microbial communities and environmental exposures, making them harder to tell apart.